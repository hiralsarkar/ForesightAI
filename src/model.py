...
from src.features import TARGET, Scope, monotone_vector, serving_feature_set
...
. `all` trains on every attribute and is for benchmarking only -- a model
    ...
    if feature_set == "screener":
        ...
        from src.scoring import screener_feature_set
...
from src.features import TARGET, Scope
...
from src.features import BY_ID, Category, label_for
...
def monotonicity_violations(
    ...
    **This is the demo-safety check.** Any metric exposed as a slider must come back
    ...
    """
    from src.features import Direction, direction_for
...
def audit_sliders(estimator: Any, X: pd.DataFrame, features: Sequence[str]) -> pd.DataFrame:
    ...
    from src.features import SLIDER_FEATURES
